...
import { ALMA_AVAILABLE_COUNT } from "./alma-availability";
...
export default function ProjectPicker({ projects, selected, onSelect, compact = false, label = "Choisir un programme" }: {
  ...
  return <div ref={rail} className={`${styles.picker} ${compact ? styles.compactPicker : ""}`} role="group" aria-label={label}>
    {projects.map((project, index) => <button type="button" key={project.id} ref={node => { buttons.current[index] = node; }}
      aria-pressed={selected === project.id} title={project.id === "jardin-alma" ? "7 lots disponibles — mise à jour BNG du 22/09/2026" : undefined} data-sold-out={project.soldOut || undefined} className={styles.projectChoice} onClick={() => onSelect(project.id)}
      onKeyDown={event => {
      ...
      <span className={styles.choiceCopy}><strong>{project.name}</strong>{project.soldOut ? <span className={styles.soldTag}>{project.delivered ? "LIVRÉ · SOLD OUT" : "SOLD OUT"}</span> : project.id === "jardin-alma" ? <span className={styles.soldTag}>{ALMA_AVAILABLE_COUNT} disponibles</span> : <small>{project.category}</small>}</span>
