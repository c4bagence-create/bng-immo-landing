...
export default function SoldOutMark({ inline = false, delivered = false, waitlist = false }: { inline?: boolean; delivered?: boolean; waitlist?: boolean }) {
  ...
  return <motion.div className={`${styles.mark} ${inline ? styles.inline : ""}`}
    ...
    <svg viewBox="0 0 230 100" role="img" aria-label={delivered ? "Livré — Sold out" : waitlist ? "Sold out — liste d’attente" : "Sold out — tous les lots vendus"}>
      ...
      <text x="115" y="77" textAnchor="middle" fill="#635747" fontFamily="Arial, sans-serif" fontSize="9" fontWeight="700" letterSpacing="1.8">{delivered ? "LIVRÉ · JUIN 2025" : waitlist ? "LISTE D’ATTENTE" : "TOUS LES LOTS VENDUS"}</text>
    </svg>
  </motion.div>;
}
